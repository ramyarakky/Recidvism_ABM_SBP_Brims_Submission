"""
run_and_chart_gender_rearrest.py
=================================
End-to-end driver:
  ...
  4. Computes year-by-year rates by Gender (Male / Female)
...
Output:
  - validation_output/chart_gender_small_multiples.png
  - validation_output/agent_cohort_pooled.csv
  - validation_output/gender_rearrest_by_year.csv
"""
...
# ── Calibrated parameters (April 2026 — year-1 integration) ──────────────────
CALIBRATED_PARAMS = {
    "Risk_Contrast_Strength":                1.0,
    "Supervision_Monitoring_Intensity":      1.1200,
    "Risk_Effect_Decay_After_1Y":            0.5240,   # FIXED BJS anchor
    "Risk_Effect_Decay_After_3Y":            0.5000,   # FIXED BJS anchor
    "Risk_Effect_Decay_After_6Y":            0.5080,   # FIXED BJS anchor
...
# ── Color system for the small-multiples chart ───────────────────────────────
PALETTE = {
    "Male": {
...
def _run_one(args):
    ...
    try:
        model = RecidivismModel(
            ...
            calibration_params    = {**get_global_calibration_params(),
                                       **CALIBRATED_PARAMS},
            seed                  = effective_seed,
        )
...
def main():
    ...
    print("="*76)
    print("  END-TO-END DRIVER \u2014 Gender x Year-of-First-Arrest Validation")
    ...
    for k, v in CALIBRATED_PARAMS.items():
        tag = "  [FIXED BJS]" if "Risk_Effect_Decay" in k else ""
        print(f"    {k:<45s} {v:.4f}{tag}")
    print("="*76)
    ...
    print(f"\n    {'Total':<8}{BJS_TABLE3['total_9yr']['Male']:>10.1%}"
          f"{rates['Male']['total_9yr']:>10.1%}"
          f"{BJS_TABLE3['total_9yr']['Female']:>12.1%}"
          f"{rates['Female']['total_9yr']:>12.1%}")

    # ── Step 4: Generate chart ──────────────────────────────────────────────
    print("\nSTEP 4: Generating small-multiples chart...")
    chart_path = os.path.join(CONFIG["output_dir"],
                                "chart_gender_small_multiples.png")
    plot_small_multiples(rates, chart_path)

    # ── Step 5: Save CSV ────────────────────────────────────────────────────
    print("\nSTEP 5: Saving CSV...")
    save_gender_csv(rates, CONFIG["output_dir"])
