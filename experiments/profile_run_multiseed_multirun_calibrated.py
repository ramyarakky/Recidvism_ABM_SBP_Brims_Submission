...
from recidivism_abm.config.risk_config import get_global_calibration_params
...
BASE_PARAMS = {
    ...
    "output_directory":"baseline_unbiasedmodel_calibrated_output"
...
def build_tasks(n_runs: int, seeds_per_run: int, base_seed: int = 1000,
    ...
    """
    cal = calibration_params if calibration_params is not None else get_global_calibration_params()
    tasks: List[Dict] = []
...
def plot_all(
    ...
    n_runs: int,
    seeds_per_run: int,
    out_dir: str = "baseline_calibrated_output",
) -> None:
...
if __name__ == "__main__":
    ...
    OUT_DIR = "baseline_calibrated_output"
    os.makedirs(OUT_DIR, exist_ok=True)
